"use strict";
/**
 * A solver for the Apollonian circle problem:
 *   find the circle that touches three other circles in extactly one point (each).
 * The solution is not necesarily unique.
 *
 * For each of the three circles a boolean attribute can be passed that determines if the
 * resultint circle should touch the input circle should be touched on the inside or outside.
 *
 * Not that there can be condition when the circle cannot be calculated (e.g. all circles
 * have the same size and are colinear).
 *
 * Found at
...
exports.solveApollonius3 = void 0;
var Circle_1 = require("../../Circle");
var Vertex_1 = require("../../Vertex");
var solveApollonius3 = function (c1, c2, c3, circle1Inside, circle2Inside, circle3Inside) {
    // https://github.com/DIKU-Steiner/ProGAL/blob/master/src/ProGAL/geom2d/ApolloniusSolver.java
    var x1 = c1.center.x;
    var y1 = c1.center.y;
    var r1 = c1.radius;
    ...
    var y3 = c3.center.y;
    var r3 = c3.radius;
    var s1 = Boolean(circle1Inside) ? 1 : -1;
    var s2 = Boolean(circle2Inside) ? 1 : -1;
    var s3 = Boolean(circle3Inside) ? 1 : -1;
    // Currently optimized for fewest multiplications.
    var v11 = 2 * x2 - 2 * x1;
    var v12 = 2 * y2 - 2 * y1;
    var v13 = x1 * x1 - x2 * x2 + y1 * y1 - y2 * y2 - r1 * r1 + r2 * r2;
