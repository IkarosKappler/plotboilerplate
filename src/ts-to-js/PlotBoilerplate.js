...
var PlotBoilerplate = /** @class */ (function () {
     ...
     * @return {void}
     **/
    PlotBoilerplate.setCSSscale = function (element, scaleX, scaleY) {
    ...
    PlotBoilerplate.Draggable = (_a = /** @class */ (function () {
            ...
            class_1.prototype.costructor = function (item, typeName) {
                this.item = item;
            ...
            ;
            class_1.prototype.isVertex = function () { return this.type == Draggable.VERTEX; };
            ;
            ...
            if (i + 1 > drawable.bezierCurves.length) {
                ...
                drawable.bezierCurves[drawable.bezierCurves.length - 1].endPoint.listeners.addDragListener(function (e) {
                    if (!drawable.adjustCircular) {
                        ...
                        drawable.moveCurvePoint(cindex * 1, drawable.END_CONTROL_POINT, { x: e.params.dragAmount.x / 2, y: e.params.dragAmount.y / 2 });
                    }
                    drawable.updateArcLengths();
                });
            }
...
;
locatePointNear(point, Vertex, tolerance ?  : number);
Draggable | boolean;
{
    // var tolerance = 7;
...
handleClick(x, number, y, number);
{
    ...
    if (p) {
        if (keyHandler.isDown('shift')) {
            if (p.type == 'bpath') {
                var vert_1 = _self.paths[p.pindex].bezierCurves[p.cindex].getPointByID(p.pid);
                if (vert_1.attr.selectable)
                    vert_1.attr.isSelected = !vert_1.attr.isSelected;
            }
            else if (p.type == 'vertex') {
                var vert_2 = _self.vertices[p.vindex];
                if (vert_2.attr.selectable)
                    vert_2.attr.isSelected = !vert_2.attr.isSelected;
            }
            _self.redraw();
        }
        ...
        var vert_3 = _self.transformMousePosition(x, y);
        _self.selectPolygon.vertices.push(new Vertex(vert_3.x, vert_3.y));
        _self.redraw();
    }
}
/**
 ...
 **/
transformMousePosition(x, number, y, number);
XYCoords;
{
...
;
mouseDownHandler(e, XMouseEvent);
{
...
;
mouseDragHandler(e, XMouseEvent);
{
...
;
mouseWheelHandler(e, XMouseEvent);
{
...
dat.gui.GUI;
{
    ...
    _addDrawConfigElement(fold, basePath, any, conf, any);
    {
        for (var i in conf) {
