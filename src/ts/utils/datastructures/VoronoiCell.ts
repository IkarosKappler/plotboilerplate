...
export class VoronoiCell {
  ...
  constructor(triangles: Array<Triangle>, sharedVertex: Vertex, sharedVertexIndex: number) {
    ...
    if (typeof sharedVertex === "undefined") sharedVertex = new Vertex(0, 0);
  ...
  toPathArray(): Array<Vertex> {
    if (this.triangles.length == 0) return [];
    if (this.triangles.length == 1) return [this.triangles[0].getCircumcircle().center];
    const arr: Array<Vertex> = [];

    // Working for path begin
    if (false && this.isOpen()) arr.push(VoronoiCell._calcOpenEdgePoint(this.triangles[0], this.triangles[1], this.sharedVertex));
  ...
  private static _calcOpenEdgePoint(tri: Triangle, neigh: Triangle, sharedVertex: Vertex): Vertex {
    ...
    const openEdgePoint: Vertex = new Vertex(
      perpendicular.x + (center.x - perpendicular.x) * 1000,
