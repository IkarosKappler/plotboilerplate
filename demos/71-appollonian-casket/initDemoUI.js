...
(function (_context) {
  _context.initDemoUI = function (appContext, POINT_SET_TYPES) {
    ...
    // +-------------------------------
    {
      ...
      // prettier-ignore
      foldApoll.add(appContext.config, "iterations").min(0).max(32).step(1).title("The number of iterations").onChange(function () { appContext.pb.redraw(); });
      // prettier-ignore
      foldApoll.add(appContext.config, "drawPerpendiculars").title("Draw the perpendiuculars whre circles touch.").onChange(function () { appContext.pb.redraw(); });
