  ...
  window.addEventListener("load", function () {
    ...
    var locateFnById = function (id) {
      for (var i = 0; i < functionCache.length; i++) {
        if (functionCache[i].id == id) return i;
      }
      return -1;
    };
    ...
    var addFunction = function (expression) {
      ...
      left.insertBefore(container, document.getElementById("addFnBtn"));
    ...
    var installInputListeners = function (fnElement) {
      ...
      fnElement.inputElement.addEventListener("change", function (event) {
        ...
        try {
          ...
          if (!parsed) {
            if (humane) humane.log("Parse Error: " + term);
            return;
          }
          var index = locateFnById(fnElement.id);
          functionCache[index].parsedFunction = function (x) {
            return parsed.evaluate({ x: x });
          };
          pb.redraw();
        } catch (err) {
          console.log(err.message);
          if (humane) humane.log('Parse Error: "' + term + '". ' + err.message);
        }
      });
    ...
    var rulerV = new Ruler(inputRangeX.min, inputRangeY.min, inputRangeY.max, 0.0, 0.25, "vertical", 20);
    var rulerH = new Ruler(
      inputRangeY.min + inputRangeY.length() / 2, // middle of interval?
      inputRangeX.min,
      inputRangeX.max,
      0.0,
      0.25,
    ...
    var functionDraw = function () {
      ...
      var croppedInputRangeX = new Interval(Math.max(inputRangeX.min, viewport.min.x), Math.min(inputRangeX.max, viewport.max.x));
      for (var i = 0; i < functionCache.length; i++) {
        evalFunction(functionCache[i].parsedFunction, croppedInputRangeX, functionCache[i].color);
      }
    };
    ...
    // +-------------------------------
    {
      ...
      f0.add(config, "stepCount")
        .min(1)
        .max(1028)
        .step(10)
        .title("Number of steps on the x axis.")
        .onChange(function () {
          pb.redraw();
        });
      f0.add(config, "scaleY")
        .min(0.25)
        .max(2.0)
        .step(0.05)
      ...
      f0.add(config, "lineWidth")
        .min(1)
        .max(20)
        .title("The line with to use.")
        .onChange(function () {
          pb.redraw();
        });
