  ...
  window.initializePB = function () {
    ...
    for (var i = 0; i < 2; i++) {
      ...
      var circle = new Circle(
        center,
        i == 0 ? Math.abs(randomVertex().x) : circles[i - 1].center.distance(center) * Math.random() * 1.2
      );
      circles[i] = circle;
    ...
    // +-------------------------------
    new MouseHandler(pb.canvas, "twocircle-demo").move(function (e) {
    ...
    // +-------------------------------
    {
      ...
      gui
        .add(config, "drawRadicalLine")
        .onChange(function () {
          pb.redraw();
        })
        .name("drawRadicalLine")
        .title("Draw the radical line?");
      gui
        .add(config, "drawIntersectionPoints")
        .onChange(function () {
          pb.redraw();
        })
        .name("drawIntersectionPoints")
        .title("Draw the intersection points?");
      gui
        .add(config, "drawSectorLines")
        .onChange(function () {
          pb.redraw();
        })
        .name("drawSectorLines")
        .title("Draw the sector lines of circle sections?");
    }
