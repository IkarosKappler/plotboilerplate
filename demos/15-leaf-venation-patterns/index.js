...
(function (_context) {
  ...
  window.addEventListener("load", function () {
    ...
    var postDraw = function () {
      ...
      drawBezierPath(scaledPath, "rgba(128,128,128,.5)", 1, function (v) {
        return v;
      });
      drawBezierPath(scaledPath, "rgba(128,128,128,.5)", 1, function (v) {
        return v.clone().invX();
      ...
      if (config.drawAuxins) {
        ...
        for (var a in auxinSources.vertices) {
          ...
          //if( !auxin )
          //    return;
          if (scaledBoundingPolygon.containsVert(auxin)) pb.fill.circle(auxin, config.growthSize, "rgba(255,96,0,0.25)");
        }
      }
      ...
      // Draw the sub path
      drawBezierPath(scaledSubPath, "rgba(0,0,255,0.5)", 2, function (v) {
        return v.clone().invX();
      });
    }; // END postDraw
    ...
    var drawBezierPath = function (path, color, lineWidth, vertexTransform) {
      for (var c in path.bezierCurves) {
        pb.draw.cubicBezier(
          vertexTransform(path.bezierCurves[c].startPoint),
    ...
    for (var i in leafShape.path.bezierCurves) {
      if (i > 0) leafShape.path.bezierCurves[i].startPoint.attr.bezierAutoAdjust = true;
      if (i + 1 < leafShape.path.bezierCurves.length) leafShape.path.bezierCurves[i].endPoint.attr.bezierAutoAdjust = true;
    }
    ...
    var clearAuxinsFromNodes = function () {
      ...
      for (var a in auxinSources.vertices) {
        ...
        for (var i in nodeList) {
          var node = nodeList[i];
          if (node.distance(auxin) < config.growthSize) remove = true;
        }
        if (!remove) newAuxinList.push(auxin);
      }
      auxinSources.vertices = newAuxinList;
    };

    var auxinListToNormalizedSum = function (node, auxinIndexList) {
      if (auxinIndexList.length == 0) return null;
    ...
    var mapAuxinsToClosestNodes = function () {
      for (var a in auxinSources.vertices) {
        ...
        if (!scaledBoundingPolygon.containsVert(auxin)) continue;
        // Find the closest node to this auxin
        var closestn = 0;
        for (var n in nodeList) {
          if (nodeList[n].distance(auxin) < nodeList[closestn].distance(auxin)) closestn = n;
        }
    ...
    var growVenes = function () {
      ...
      var newNodeList = [];
      for (var n in nodeList) {
        if (typeof nodeAffectingAuxins[n] == "undefined" || nodeAffectingAuxins[n].length == 0) continue;
    ...
    function renderAnimation() {
      ...
      if (config.animateGrowth && iterno <= growthSteps) window.requestAnimationFrame(renderAnimation);
      // Animation stopped
      else config.animateGrowth = false;
    }

    function animateStep() {
      if (nodeList.length == 0) initGrowth();
      growVenes();
    }
    ...
    // +-------------------------------
    {
      ...
      f0.add(config, "leafScale")
        ...
        .listen()
        .name("Leaf Scale")
        .title("Scale the leaf path.");
      f0.add(config, "auxinCount")
        .min(1)
        .max(1000)
        .step(1)
        .onChange(function () {
        ...
        .name("#Auxins")
        .title("Change auxin count.");
      f0.add(config, "drawAuxins")
        .onChange(function () {
          pb.redraw();
        })
        .name("Draw auxins")
        .title("Draw auxins.");
      f0.add(config, "animateGrowth")
        .onChange(function () {
          toggleAnimation();
        })
        .name("Animate")
        .title("Animate.")
        .listen()
        .arrowBounce("Click here", { fadeDelay: 5000, detachAfter: 6000 });
      f0.add(config, "animateStep").name("Animate Step").title("Animate next step.");
      f0.add(config, "growthSize")
        .onChange(function () {
          pb.redraw();
        })
        .name("Growth size")
        .title("The amount of growth in each step.");
