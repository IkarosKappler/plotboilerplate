...
(function (_context) {
  ...
  window.initializePB = function () {
    ...
    var drawAll = function () {
      var convexHull = new Polygon(
        getConvexHull(pointList.pointList),
        false // isOpen=false
      );

      if (config.drawConvexHull) pb.draw.polyline(convexHull.vertices, false, "rgba(0,192,255,1.0)", 2);
    ...
    var drawInnerBisectors = function (convexHull) {
      ...
      for (var i = 1; i <= convexHull.vertices.length; i++) {
        var bisector = geomutils.nsectAngle(
          convexHull.vertices[i % n],
          convexHull.vertices[(i - 1) % n],
          convexHull.vertices[(i + 1) % n],
          2
        )[0];
    ...
    // +-------------------------------
    {
      ...
      gui
        ...
        .title("Point count");
      gui
        .add(config, "drawConvexHull")
        .onChange(function () {
          pb.redraw();
        })
        .name("Draw Convex Hull")
        .title("Draw the Convex Hull.");
      gui
        .add(config, "animate")
        .onChange(function () {
          toggleAnimation();
