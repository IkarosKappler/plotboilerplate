...
(function (_context) {
  ...
  window.addEventListener("load", function () {
    ...
    bpath[1] = [
      bpath[0][1].clone(), // Use same reference
      new Vertex(0, diameter),
      new Vertex(diameter, diameter * fract),
      new Vertex(diameter * fract, diameter)
    ];
    bpath[2] = [
      bpath[1][1].clone(), // Use same reference
      new Vertex(-diameter, 0),
      new Vertex(-diameter * fract, diameter),
      new Vertex(-diameter, diameter * fract)
    ];
    bpath[3] = [
      bpath[2][1].clone(), // Use same reference
    ...
    for (var i = 0; i < bpath.length; i++) {
      for (var e = 0; e < 4; e++) {
        timeline.fromTo(
          ...
          {
            x: bpath[(i + 2) % 4][map[e]].x * scale[e],
            y: bpath[(i + 2) % 4][map[e]].y * scale[e],
            yoyo: true,
            repeat: 1,
